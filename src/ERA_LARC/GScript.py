...
def main():
    ...
    for year in date_str_list:
        ...
        if not os.path.exists(os.path.join(pr_path, f'{year}.nc')):
            try:
                reanalysis.sel(latitude=slice(lat_max,lat_min), longitude=slice(lon_min,lon_max)).sel(time=year)[['total_precipitation']].to_netcdf(os.path.join(pr_path, f'{year}.nc'))
            except:
                reanalysis.sel(latitude=slice(lat_max,lat_min), longitude=slice(lon_min,lon_max)).sel(valid_time=year)[['total_precipitation']].rename({'valid_time': 'time'}).to_netcdf(os.path.join(pr_path, f'{year}.nc'))
    ...
    if not os.path.exists(os.path.join(daily_path, f'pr.nc')):
        os.system(f'cdo chname,total_precipitation,pr -daysum -mergetime {os.path.join(pr_path, "*.nc")} {os.path.join(daily_path, "pr.nc")}')
